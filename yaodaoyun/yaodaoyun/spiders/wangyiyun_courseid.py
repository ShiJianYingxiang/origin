...
class WangyiyunSpider(scrapy.Spider):
        ...
        if result:
            for result_temp in result:
                ...
                for item in content_lists:
                    ...
                    yield scrapy.Request(url=detail_url, headers=self.get_headers, meta={'category_id': category_id,'categroy_data': item}, callback=self.detail_item)
    ...
    def detail_item(self, response):
        category_id = response.meta.get('category_id', '')
        datas = response.meta.get('categroy_data', '')


        courseId = re.search('introduction/(\d+)\.htm', response.url).group(1)   #获取详情页ID

        payload = "callCount=1\r\nscriptSessionId=${scriptSessionId}190\r\nhttpSessionId=\r\nc0-scriptName=PlanNewBean\r\nc0-methodName=getPlanCourseDetail\r\nc0-id=0\r\nc0-param0=string:%s\r\nc0-param1=number:0\r\nc0-param2=null:null\r\nbatchId=1591429741178" % courseId
        yield scrapy.Request(url=self.lesson_url, headers=self.lesson_headers, body=payload, method='post',
                             meta={'courseid': courseId, 'category_id': category_id}, callback=self.detail_lesson_parse)

        breadcrumb_dict_content = {}  #解析导航
        breadcrumb = response.xpath('''//ul[@class="g-flow"]//li[contains(@class,"navcrumb")]//a''')
        ...
        for temp_key, temp_value in datas.items():

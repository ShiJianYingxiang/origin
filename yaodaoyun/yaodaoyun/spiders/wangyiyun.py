...
class WangyiyunSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for category_id in category_id_list:
            ...
            self.logger.info('----请求分类---{}----'.format(category_id))
            ...
            for result_temp in result:
                ...
                for item in content_lists:
                    ...
                    yield scrapy.Request(url=self.lesson_url, headers=self.lesson_headers, body=payload, method='post',
                                         meta={'courseid': courseId, 'category_id': category_id},
                                         callback=self.detail_lesson_parse, dont_filter=True)
    ...
    def detail_lesson_parse(self,response):
        ...
        self.logger.info('----开始解析课次详情---{}---'.format(courseid))

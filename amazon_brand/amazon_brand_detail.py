...
def download(url, brand_key):
    ...
    if success:
        ...
        if not url_db.sismember('amazon_brand:failed_{}_set'.format(brand_key), pid):
            url_db.sadd('amazon_brand:failed_{}_set'.format(brand_key), pid)
            url_db.lpush("amazon_brand:failed_{}_list".format(brand_key), pid)
...
def parse_list(url, brand_key):
    ...
    brand_name = brand_name.replace('Visit the', '').replace('Store', '').replace('brand_name', '').replace('：', '').replace(':', '').replace('Brand', '').strip()
    ...
    # print(content_dict)
    url_db.sadd('amazon_brand:successful_{}_set'.format(brand_key), pid)
...
if __name__ == '__main__':
    ...
    msg = '亚马逊品牌anker队列已消耗完毕，采集成功{}个'.format(str(url_db.scard('amazon_brand:successful_anker_set')))
    dingding(msg)
    ##############
    msg1 = '亚马逊品牌aukey队列已消耗完毕，采集成功{}个'.format(str(url_db.scard('amazon_brand:successful_aukey_set')))
    dingding(msg1)

...
def download(url, brand_key):
    ...
    if success:
        ...
        if not url_db.sismember('amazon_brand_spider:failed_brand_{}_set'.format(brand_key), pid):
            url_db.sadd('amazon_brand_spider:failed_brand_{}_set'.format(brand_key), pid)
            url_db.lpush("amazon_brand_spider:failed_brand_{}_list".format(brand_key), pid)
...
def parse_list(url, brand_key):
    content_dict = dict()
    resp = download(url, brand_key)
    ...
    #     print('*****MEOW不在源码里******')
    #     url_db.lpush("amazon_detail_spider:403_list", pid)
    ...
    try:
        if 'Currently unavailable' in resp.text or 'Available from' in resp.text:
            ...
            price = content.xpath(
                '''//div[@id="buyNew_noncbb"]//span/text()|//div[@id="cerberus-data-metrics"]//@data-asin-price|//div[@id="price"]//span[@id="priceblock_ourprice"]/text()|//span[@id="price_inside_buybox"]/text()''')[
                0]
            sale_status = 1
    except:
    ...
    other_bsr = dict()
    xx = ''
    other_bsr_parse = content.xpath(
        '''//ul[contains(@class,"detail-bullet-list")]//a[contains(@href,'bestsellers')]''')
    ...
    if 'Sellers Rank' in resp.text or 'sellers rank' in resp.text:
        ...
        # if not url_db.sismember('amazon_detail_spider:no_bsr_set', pid):
        #     url_db.sadd('amazon_detail_spider:no_bsr_set', pid)
        #     url_db.lpush('amazon_detail_spider:no_bsr_list', pid)
    print('%%%%%%%%%%%%%%%%{}%%%%%%%%%%%%%%%%%%%'.format(bsr))
    ...
    brand_id = ''  # 品牌id
    batch_tm = time.strftime('%Y-%m-%d', time.localtime(time.time()))  # 批次时间  #####
    # 未找到的字段---添加商品列表解析
    sku_list = []  # sku商品列表
    sku_id = ''
    try:
        ...
        if spu_id:
            ...
            for sku_id_value in sku_id_dict.values():
                sku_list.append(sku_id_value)
    except:
        spu_id = pid
        sku_list = []
    model = ''  # 款式参数
    ...
    # print(content_dict)
    url_db.sadd('amazon_brand_spider:successful_{}_list'.format(brand_key), pid)

    with open('/mnt/data/weidong.shi/file/amazon/product_info/hive_{}_product_info_'.format(brand_key) + time.strftime('%Y-%m-%d') + '.txt', 'a+', encoding='utf-8') as ff:
    # with open('hive_{}_product_info_'.format(brand_key) + time.strftime('%Y-%m-%d') + '.txt', 'a+',encoding='utf-8') as ff:
...
def get_data():
    ...
    while 1:
        # key_list = [, 'aukey']
        # brand_key = 'anker'
        redis_dict = url_db.lpop('amazon_brand_spider:hive_{}_list'.format(brand_key))  #
        if redis_dict:
            if isinstance(redis_dict, bytes):
                product_id = redis_dict.decode()
                product_url = 'https://www.amazon.com/dp/{}'.format(product_id)
                print(product_url)
                job.append(exe_pool.spawn(parse_list, product_url, brand_key))
        else:
            print('队列已消耗完毕，退出')
            msg = '亚马逊品牌{}队列已消耗完毕，采集成功{}个'.format(brand_key, str(
                url_db.scard('amazon_brand_spider:successful_{}_list'.format(brand_key))))
            dingding(msg)

            break

    # product_id = 'B00ENE3WJ0'  #B07MHKNQFY\B00ENE3WJ0
    # product_url = 'https://www.amazon.com/dp/{}'.format(product_id)

...
def download(url, product_data):
    ...
    if success:
        ...
        if not url_db.sismember('amazon_detail_spider:failed_urls_set', json.dumps(product_data)):
            url_db.sadd('amazon_detail_spider:failed_urls_set', json.dumps(product_data))
            url_db.lpush("amazon_detail_spider:failed_urls_list", json.dumps(product_data))
...
def parse_list(url, product_data):
    ...
    try:
        ...
        if not url_db.sismember('amazon_import:failed_set', json.dumps(product_data)):
            url_db.sadd('amazon_import:failed_set', json.dumps(product_data))
            url_db.lpush("amazon_import:failed_list", json.dumps(product_data))
        return
    ...
    if shop_url:
        shop_url = 'https://www.amazon.com' + shop_url if 'http' not in shop_url else ''
    ...
    if 'Sellers Rank' in resp.text or 'sellers rank' in resp.text:
        ...
        if not url_db.sismember('amazon_import:no_bsr_set', json.dumps(product_data)):
            url_db.sadd('amazon_import:no_bsr_set', json.dumps(product_data))
            url_db.lpush('amazon_import:no_bsr_list', json.dumps(product_data))
        return None
    print('%%%%%%%%%%%%%%%%{}%%%%%%%%%%%%%%%%%%%'.format(bsr))
    ...
    # print(content_dict)
    url_db.lpush('amazon_import:successful_list', pid)

    with open('/mnt/data/weidong.shi/file/amazon/product_info/comment50_' + time.strftime('%Y-%m-%d') + '_' + str(os.getpid()) + '.txt', 'a+', encoding='utf-8') as ff:
